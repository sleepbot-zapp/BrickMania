...
class Ball:
    ...
    def draw_ball(
        ...
        for i, (tx, ty) in enumerate(trail):
            alpha: typing.Union[int, float] = max_alpha - int(
                max_alpha * (i / trail_length)
            )
            trail_width: typing.Union[int, float] = max_trail_width - (
                max_trail_width - min_trail_width
            ) * (i / trail_length)
            dx: typing.Union[int, float] = (
                (i / trail_length) * divergence * self.ball_radius
            )
    ...
    def _handle_paddle_collision(self, player: Player):
        ...
        if (
            ...
            relative_intersection_x = (self.x - paddle_center) / (
