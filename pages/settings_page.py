...
class Settings(Page):
    ...
    def display(self) -> bool:
        ...
        while True:
            ...
            for i, option in enumerate(self.settings_options):
                c = self.game.colors.YELLOW if i == self.selected_option else self.game.colors.WHITE
    ...
    def display_music_settings(self) -> None:
        ...
        while True:
            ...
            options_font = self.fonts[1]
            volume_label_text = options_font.render("Volume:", True, self.game.colors.WHITE)
    ...
    def display_color_settings(self) -> None:
        ...
        while True:
            ...
            for i, color_name in enumerate(self.color_keys):
                ...
                pygame.draw.rect(
                    self.screen,
                    color_value,  # Fill the rectangle with the color
                    (self.width // 2 + 200, 140 + i * 50, 40, 40)
                )

                # Draw a border around the color preview rectangle
                pygame.draw.rect(
                    self.screen,
                    (0, 0, 0),  # Border color (can be any color)
                    (self.width // 2 + 200, 140 + i * 50, 40, 40),
                    2  # Border width (2 pixels)
                )
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key in [pygame.K_LSHIFT, pygame.K_RSHIFT]:
                        ...
                        if event.key == pygame.K_RIGHT:
                            self.color_edit_component = (self.color_edit_component + 1) % 3
                        elif event.key == pygame.K_LEFT:
                            self.color_edit_component = (self.color_edit_component - 1) % 3
                        elif event.key == pygame.K_UP:
                            selected_color[self.color_edit_component] = min(255, selected_color[self.color_edit_component] + 1)
